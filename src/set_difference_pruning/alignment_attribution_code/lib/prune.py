...
def jaccard_index(tensor1: torch.Tensor, tensor2: torch.Tensor) -> float:
    ...
    # Find intersection and union using PyTorch operations
    intersection = torch.cat((unique1, unique2), dim=0)
    intersection = torch.unique(intersection, sorted=False, return_counts=True)
    intersection_count = torch.sum(intersection[1] > 1)

    union = torch.cat((unique1, unique2), dim=0)
    union = torch.unique(union, sorted=False).numel()

    # Compute the Jaccard Index
    jaccard = intersection_count.item() / union

    return jaccard

def get_optimal_sparsity(args, W_metric1: torch.Tensor, W_metric2: torch.Tensor) -> float:
    # Use bayesian optimization to compute the most optimal sparsity level per layer
    from bayes_opt import BayesianOptimization
    bounds = {"sparsity": (-4, 0)}

    def calculate_overlap_neg(sparsity):
        sparsity_raw = np.power(10,sparsity)
        topk = int(W_metric1.numel()*sparsity_raw)

        _, util_idx = W_metric1.flatten().topk(topk)
        _, safety_idx = W_metric2.flatten().topk(topk)

        # -1*np.power(jaccard_index(util_idx, safety_idx),(1./4.)*sparsity + 1.) # make sparsity as large as possible
        return -1*jaccard_index(util_idx, safety_idx)*((1./4.)*sparsity_raw + 1.)

    optimizer = BayesianOptimization(
        f=calculate_overlap_neg,
        pbounds=bounds,
    ...
    optimal_sparsity = optimizer.max["params"]["sparsity"]

    optimal_sparsity = np.power(10, optimal_sparsity)
...
def prune_ratio(
    ...
    if args.use_diff or args.recover_from_base:
        assert model_base is not None
        layers_base = model_base.model.layers
    metric1 = args.util_data #"alpaca_cleaned_no_safety"
    metric2 = prune_data

    print(
        "prune metric1 = {}, metric2 = {}".format(
            metric1, metric2
        )
    )
    if args.prune_part:
        ...
        for name in subset:
            ...
            try:
                W_metric1 = pickle.load(
                    open(
                        f"{experiment_path}/{args.model}/unstructured/wandg/{metric1}/wanda_score/W_metric_layer_{i}_name_model.layers.{i}.{name}_weight.pkl",
                        "rb",
                    )
                )
                W_metric2 = pickle.load(
                    open(
                        f"{experiment_path}/{args.model}/unstructured/wandg/{metric2}/wanda_score/W_metric_layer_{i}_name_model.layers.{i}.{name}_weight.pkl",
                        "rb",
                    )
                )
            except:
            ...
            threshold = args.prune_ratio_threshold
            ratios = W_metric1 / W_metric2
            mask = ratios < threshold # we do not want to modify weights that are more important for utility than pruning
            modified_ratios = (ratios / threshold)**4 # this way the neurons at the threshold are not modified and ones below threshold always have ratio less than 1

            subset[name].weight.data[mask] *= modified_ratios[mask] ## set weights to zero
...
def prune_wandg_set_difference(
    ...
    for i in range(len(layers)):
        ...
        # filter layers
        if layer_subset == "mlp_only":
            subset = {name : subset[name] for name in subset if "mlp" in name}
        elif layer_subset == "omit_keys_queries":
            subset = {name : subset[name] for name in subset if "k_proj" not in name and "q_proj" not in name}
        ...
        if not args.prune_part:
            for name in subset:
                ...
                try:
                    W_metric1 = pickle.load(
                        open(
                            f"{directory}/{args.model}/unstructured/{scoring_method}/{metric1}/wanda_score/W_metric_layer_{i}_name_model.layers.{i}.{name}_weight.pkl",
                            "rb",
                        )
                    )
                    W_metric2 = pickle.load(
                        open(
                            f"{directory}/{args.model}/unstructured/{scoring_method}/{metric2}/wanda_score/W_metric_layer_{i}_name_model.layers.{i}.{name}_weight.pkl",
                            "rb",
                        )
                    )
                except:
                ...
                if dynamic_sparsity:
                    optimal_sparsity = get_optimal_sparsity(args, W_metric1, W_metric2)
                    p, q = optimal_sparsity, optimal_sparsity
                ...
                if args.recover_from_base:
                    ...
                    # # this is the correct way to do pruning, otherwise backward() doesn't work
                    # copy = subset[name].weight.data.clone()
                    # copy[W_mask] = 0
                    # subset[name].weight = torch.nn.Parameter(copy, requires_grad=True)
                    # this makes running 2nd iter of pruning impossible
                    subset[name].weight.data[W_mask] = 0  ## set weights to zero
        else:
            # args.prune_part == True. We only prune the layer with low jaccard index, which is:
            # layer 0 mlp_down_proj
